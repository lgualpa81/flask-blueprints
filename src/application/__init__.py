...
def async_monitor_tracer(_logname, _loglevel, _payload):
    ...
    try:
        ...
        _pl_notification = {"event": "cluster_commerce_microservice_tracer",
...
def register_handler_exceptions(app):
    ...
    def log_request(response):
        ...
        log_name = "tracer-microservice-payments"
        klog = {
            "log_name": log_name,
            "level": log_level,
            "service": {
...
def create_app():
    ...
    from .paylink import paylink_bp
    from .logdb import logdb_bp
    from .backoffice import backoffice_bp

    app.register_blueprint(paylink_bp)
    app.register_blueprint(logdb_bp)
    app.register_blueprint(backoffice_bp)

    # version 2
    from .paylink.v2 import paylink_v2_bp
    app.register_blueprint(paylink_v2_bp, url_prefix='/paylink/v2')

    from .balance.v1 import balance_v1_bp
    app.register_blueprint(balance_v1_bp, url_prefix='/balance/v1')

    # mocks
    from .mocks.v1 import mock_bp
    app.register_blueprint(mock_bp, url_prefix='/mocks/v1')
